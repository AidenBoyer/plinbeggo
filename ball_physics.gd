...
func _integrate_forces(state):
	if(state.get_contact_count()):
		if(state.get_contact_collider_object(0).get_parent().has_meta(&"type") && state.get_contact_collider_object(0).get_parent().get_meta(&"type") == "Bumper"):
			...
			state.apply_impulse(state.get_contact_local_normal(0) * bumperBounciness, state.get_contact_collider_object(0).position)
			$BumperNoise.play()
		else:
			state.apply_impulse(state.get_contact_local_normal(0) * defaultBounciness, state.get_contact_collider_object(0).position)
